...
class PolygonField(serializers.Field):
        ...
        if value and isinstance(value, list):
            # try:
                ...
                for i in value:
                    latitude = i.get("lat")
                    longitude = i.get("lng")
                    result_list.append((longitude,latitude))
                
                return Polygon(result_list,srid=self.srid)
    ...
    def to_representation(self, value):
        ...
        if isinstance(value, Polygon):
            print (value)
            result = []
            ziped_value = zip(value[0].x,value[0].y)
            for i in ziped_value:
                result.append ( {
                    "lat": i[1],
                    "lng": i[0]
                })
